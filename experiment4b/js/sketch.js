...
let rockImage2;
let crab;
const worldWidth = 20; // Total number of tiles in the width of the world
const worldHeight = 15; // Total number of tiles in the height of the world

function p3_preload() {
  treeImage = loadImage(
    "https://cdn.glitch.global/53c866d9-8bc3-4d81-90e2-bbe4ecb178d1/tree.png?v=1714175967558"
  );
  rockImage1 = loadImage(
    "https://cdn.glitch.global/53c866d9-8bc3-4d81-90e2-bbe4ecb178d1/rock1.png?v=1714179988254"
  );
  rockImage2 = loadImage(
    "https://cdn.glitch.global/53c866d9-8bc3-4d81-90e2-bbe4ecb178d1/rock2.png?v=1714180323144"
  );
  crab = loadImage(
    "https://cdn.glitch.global/53c866d9-8bc3-4d81-90e2-bbe4ecb178d1/crab.png?v=1714189332515"
  );
}

function p3_setup() {
  for (let i = 0; i < initialCrabCount; i++) {
    spawnCrab();
  }
}

const initialCrabCount = 5;

let worldSeed;

function p3_worldKeyChanged(key) {
  worldSeed = XXH.h32(key, 0); // Update the world seed
  noiseSeed(worldSeed);
  randomSeed(worldSeed);

  // Reset the crabs array to clear existing crab data
  crabs = [];
}
...
function p3_drawBefore() {
  if (frameCount % 60 === 0) {
    // Spawn a crab every second if your frame rate is 60 fps
    spawnCrab();
  }
  updateCrabMovement();
}
...
function p3_drawTile(i, j) {
  ...
  let elevation = noise(i * 0.1, j * 0.1);
  let depthNoise = noise(i * 0.05, j * 0.05);
  const waterLevel = 0.5;
  const transitionRange = 0.1;
  const sandLevel = waterLevel + transitionRange;

  let tileColor;
  let drawTree = false;
  let drawRock = false;
  let drawCrab = crabs.some((crab) => crab.i === i && crab.j === j);

  let treeHash = XXH.h32(`${i}-${j}-tree`, worldSeed).toNumber();
  let rockHash = XXH.h32(`${i}-${j}-rock`, worldSeed).toNumber();

  if (elevation < waterLevel - transitionRange) {
    tileColor = lerpColor(color(135, 206, 235), color(25, 25, 112), depthNoise);
    drawRock = rockHash % 100 < 3; // Lower chance for rocks in water
  } else if (elevation > sandLevel) {
    tileColor = color(194, 178, 128); // Sand
    drawTree = treeHash % 100 < 15;
    if (!drawTree) {
      drawRock = rockHash % 100 < 7; // Higher chance for rocks on sand
    }
  } else {
    tileColor = lerpColor(
      color(72, 209, 204),
      color(194, 178, 128),
      map(elevation, waterLevel - transitionRange, sandLevel, 0, 1)
    );
  }
  ...
  if (drawTree) {
    image(treeImage, 0, -th, treeImage.width, treeImage.height);
  }
  if (drawRock) {
    let rockType = rockHash % 2 === 0 ? rockImage1 : rockImage2;
    image(rockType, 0, -th / 2, rockType.width, rockType.height);
  }
  if (drawCrab) {
    image(crab, 0, 0, crab.width / 2, crab.height / 2);
  }
}
...
function spawnCrab() {
  let i = Math.floor(random(0, worldWidth));
  let j = Math.floor(random(0, worldHeight));
  crabs.push({ i, j, nextMove: millis() + random(1000, 3000) }); // Set initial nextMove
  
}

function updateCrabMovement() {
  let currentTime = millis();
  crabs.forEach((crab, index) => {
    if (currentTime >= crab.nextMove) {
      let directions = [
        { di: -1, dj: 0 }, // Up
        { di: 1, dj: 0 },  // Down
        { di: 0, dj: -1 }, // Left
      ...
      // Check for other crabs in adjacent tiles and remove those directions if possible
      directions = directions.filter(dir => {
        return !crabs.some(otherCrab => otherCrab !== crab && otherCrab.i === crab.i + dir.di && otherCrab.j === crab.j + dir.dj);
      });

      if (directions.length > 0) {
        let move = random(directions);
        let newI = crab.i + move.di;
        let newJ = crab.j + move.dj;
        if (newI >= 0 && newI < worldWidth && newJ >= 0 && newJ < worldHeight) {
          crab.i = newI;
          crab.j = newJ;
        }
        crab.nextMove = currentTime + random(1000, 3000); // Schedule next move
      }
    }
  });
}
